...
/**
 * uclogic_params_init_ugee_xppen_12pro() - initialization procedure
 * common to XP-Pen Pro series devices - YA 12Pro
 *
 * @hdev:	The HID device of the tablet interface to initialize and get
 *		parameters from. Cannot be NULL.
 * @params:	Parameters to fill in (to be cleaned with
 ...
 *	Zero, if successful. A negative errno code on error.
 */
static int uclogic_params_init_ugee_xppen_12pro(struct hid_device *hdev,
	struct uclogic_params *p)
{
	static const u8 init_packet[] = {
		0x02, 0xb0, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	...
	p->pen.desc_ptr = uclogic_rdesc_template_apply(
		uclogic_rdesc_xppen_a12p_pen_arr,
		uclogic_rdesc_xppen_a12p_pen_size,
		desc_params,
		ARRAY_SIZE(desc_params)
	);
	p->pen.desc_size = uclogic_rdesc_xppen_a12p_pen_size;
	p->pen.id = 0x02;

	rc = uclogic_params_frame_init_with_desc(
		&p->frame_list[0],
		uclogic_rdesc_xppen_a12p_frame_arr,
		uclogic_rdesc_xppen_a12p_frame_size,
		UCLOGIC_RDESC_V1_FRAME_ID
	);
	if (rc < 0) {
		hid_err(hdev, "initializing frame params failed: %d\n", rc);
...
#define VID_PID(_vid, _pid) \
	...
	case VID_PID(USB_VENDOR_ID_UGEE,
		...
		rc = uclogic_params_init_ugee_xppen_12pro(hdev, &p);
		if (rc != 0) {
			hid_err(hdev, "a12p init failed: %d\n", rc);
			goto cleanup;
		}
